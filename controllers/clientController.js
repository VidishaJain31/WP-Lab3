...
const loginControl = (request, response) => {
    ...
    if (!username || !password) {
        response.render('postLogin', {msg: "Login Failed!"});
    } else {
        if (request.session && request.session.user) {
            response.render('postLogin',{msg:`Login Successful! ${username}`});
        } else {
...
const registerControl = (request, response) => {
    ...
    clientServices.registerService(client, function(err, exists, insertedID) {
        ...
        if (exists) {
            response.render('postRegister', {msg: `registration failed. Username (${username}) already taken!`});
        } else {
            client.num_client = insertedID;
            response.render('postRegister', {msg: `registration Successful User: (${username}) !`});
        }
        response.end();
    });
};
